...
namespace CraftMyFit.Services.Interfaces
{
    ...
    public class SyncResult
    {
        ...
        public List<string> SyncedTables { get; set; } = new();
    }
...
// CloudSyncService.cs - Implementazione del servizio di sincronizzazione
...
namespace CraftMyFit.Services
{
    public class CloudSyncService : ICloudSyncService
    {
        ...
        public async Task<bool> IsCloudSyncEnabledAsync()
        {
            return await Task.FromResult(_preferenceService.GetBool("cloud_sync_enabled", false));
        }
        ...
        public async Task<bool> EnableCloudSyncAsync()
        {
            try
            {
                ...
                var syncResult = await SyncAllDataAsync();
        ...
        public async Task<SyncResult> SyncAllDataAsync()
        {
            ...
            try
            {
                ...
                var tables = new[] { "Users", "WorkoutPlans", "Exercises", "ProgressPhotos", "BodyMeasurements", "Achievements" };

                for(int i = 0; i < tables.Length; i++)
                {
                    NotifyProgressChanged(tables.Length, i + 1, $"Sincronizzazione {tables[i]}...");

                    var tableResult = await SyncTableAsync(tables[i]);
        ...
        {
            var lastSync = _preferenceService.GetDateTime("last_sync_date", DateTime.MinValue);
        ...
        private void NotifyStatusChanged(SyncStatus status, string? message = null)
        {
        ...
        private void NotifyProgressChanged(int total, int processed, string operation)
        {
